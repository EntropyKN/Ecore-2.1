...
$(function() {
	...
	$('.row').on('click', function(e){e.preventDefault()
		...
		xhr  =$.ajax({
			//async: false,
			type: "POST", url: CD+'/_m/groups2.0/groupGet.ajax.php'
...
//			,error: function(D){alert("err")}
	...
	$("#tagIN").on('keyup', function(e) {		
		...
		if($T) 	{
			...
			xhrSearch=$.ajax({
				type: "GET", url:CD+ "/_m/groups2.0/ajax.tags.php",data: "q="+$T+"&action=search&gymsCSV="+$("#gymsCSV").val()+"&usersCSV="+$("#usersCSV").val(),cache: false,dataType: 'html', //idgr="+$("#idg").val()
	...
	$("#EtagIN").on('keyup', function(e) {		
		...
		if($T) 	{
			...
			ExhrSearch=$.ajax({
				type: "GET", url: CD+"/_m/groups2.0/ajax.insighter.php",
