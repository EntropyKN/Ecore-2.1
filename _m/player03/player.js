$(function() {
	...
	$(".atta").on('click',function(e){
		...
		if ($(this).attr("data-open")!="out") {
			e.preventDefault()
			//alert ($(this).attr("data-open")+" "+$(this).attr("data-info"));
			if ($(this).attr("data-info")!="youtube") {
				//if ($(this).attr("data-majordim")=="w") ytid
				$("#attachViewerIN").html('<img src="'+$(this).attr("href")+'"  width="'+$(this).attr("data-W1")+'"  height="'+$(this).attr("data-H1")+'" />' )
				//else $("#attachViewerIN").html('<img src="'+$(this).attr("href")+'"  height="562.50"  width="auto" />' )
			}else{
				// youtube
				//alert ($(this).attr("data-ytid"))				
...
//		
	...
	var transmit=function(data) {
		...
		$("#saving").show()
		$.ajax({
			type: "POST", url: CD+"/_m/player03/ajax.player.php"
			,data: {data:data} 
...
//			$("#avatarSprite").attr("class", 		$("#avatarSprite").attr("class").replace("avatar_"+currentAvatarId+"_talk", "avatar_"+currentAvatarId)			)
			...
			$("#avatarSprite").show()
			//alert (scene);
			if (!isFinal) $loadA=loadAttach(s,scene);
			else $loadA=false;
