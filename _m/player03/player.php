...
$JSA[]=''.C_DIR.'/_m/player03/player.js?'.PAGE_RANDOM;
//$CSSA[]=C_PROTOCAL.'://fonts.googleapis.com/css?family=Cherry+Cream+Soda'; //Anton|Lalezar

$CSSA[]=''.C_DIR.'/_m/player03/player.css?'.PAGE_RANDOM;
...
$O.='<div id="player">'; // style="background: rgba(0, 0, 0, 0) url('.$imgOffice.') no-repeat scroll 0 0;"
	...
	///
		...
		$O.='<div id="debevaulating">';
			...
			$O.="</span>";	
		$O.="</div>";
			$O.='<a href="/'.C_DIRNS.'/?debrief/'.$G["idm"].'" class="btn noselect">';	//idm
...
//if ($_COOKIE["debug"]		){
if ($_SESSION["uid"]=="328"	|| $_COOKIE["debug"]		){
...
$O.="<br /><pre>".print_r($G["ss"],true)."</pre>";
}
