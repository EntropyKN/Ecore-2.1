...
if (!$DIRA[1] || !is_numeric($DIRA[1])  ||  $_SESSION["ulevel"]==0 ){
	echo("location:/?/".$DIRA[1]."__NOT_FOUND");exit();
}




$G=sql_queryt("SELECT  G.*, C.file coverPath  FROM games  G 
		LEFT JOIN covers C on G.cover_id=C.id where G.gameId='".(trim($DIRA[1]))."'");
/* 
if ( !$G['gameId']	) {
	echo "SELECT gameId, status, title, uid_creator,  createdTs, uid_editor, editTs, cover, Description
 FROM games where gameId='".(trim($DIRA[2]))."'";
	die ("NOT FOUND");
//	header("location:/?/".$DIRA[1]."__NOT_FOUND");exit();
}
