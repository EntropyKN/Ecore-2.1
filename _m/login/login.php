<?php
$CSSA[]='/'.C_DIR.'_m/login/login.css?change';
...
U.user_email  email,
	...
	if (!$T["uid"]	|| 	($T["notActiveSince"]<time() && $T["notActiveSince"]		) 	){
		...
		if 	($T["notActiveSince"]<time() && $T["notActiveSince"]		) $ERROR_MSG="Sorry ".$T["user_real_name"].",<br />your account has been temporarily suspended";
	}else{
		//unset ($_SESSION);
...
//				$O.=L_thanks.",<br />";
	...
	// login
	$O.='<form  action="/'.C_DIRNS.'/?login" method="post" class="" id="form_login" name="form_login">'; //style="display:none" /?login
		...
		if ($ERROR_MSG) $O.='<div class="ERROR_MSG">'.$ERROR_MSG.'</div>';
