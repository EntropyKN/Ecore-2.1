...
$(function() {
	...
	*/
	...
	window.css_adapt=function () { 
		...
		if (W<$desktopWidth) {
			if ($pageId!="player") $("#core,#footer,#menuFixed").css({'width':'100%'})
			...
			if ($pageId!="player") $("#core,#footer,#menuFixed").css({'width':$desktopWidth+'px'})	
		...
		if (W<800 ){ //|| H<591 ipad 1024 x 768
			...
			$("#switchCss").attr("href",''+CD+'/css/mobile.css?'+d)
			//alert ("mobile")
		/*		41
				if (W<526  ) {
			...
			$("#switchCss").attr("href", ''+CD+'/css/mobileOFF.css')
	...
	$('#core').on('click','.showPage', function (e) {e.preventDefault();
		...
		$.ajax({
			type: "POST", url: $url
			//type: "POST", url: CD+"/'+C_DIR+"/handler/browseProducts/browseProductsAjax.php"
